...
class LecturerDashboard extends Component {
    ...
    state = {
        ...
        password: "",
        role: "",
        addedBy: JSON.parse(localStorage.getItem('l-token'))['_id'],
        addedFailed: false,
        isAdded: "",
        errors: {},
        title: "",
        ...
        notificationType: "",
        receiver: "",
        sender: JSON.parse(localStorage.getItem('l-token'))['email'],
        message: "",
...
handleLecture = (e) => {
    ...
    const resultJSON = JSON.parse(e.target.id);

    this.setData('currentCourse', resultJSON);
...
onSubmitHandler = (e) => {
    ...
    if(addType === "1"){
        ...
        bodyMessage = {
            ...
            senderID: this.getData('l-token')._id,
            message,
    ...
    this.setState({
        ...
        password: "",
        role: "",
        addedBy: JSON.parse(localStorage.getItem('l-token'))['email'],
        addedFailed: false,
        isAdded: "",
        errors: {},
        title: "",
...
manageStudent = async () => {
    try {
        window.document.querySelector("#loader").style.display = "block";
            ...
            ReactDOM.render(<TableDataForLecturerManageStudent indexes = {indexes} theads = {theads} data = {res.data} handleView = {this.handleView} />, document.getElementById('lecturer'))
...
manageCourse = async () => {
    ...
    try {
        ...
        const response = res.data.filter((data) => {
            return data.lecturerID === JSON.parse(localStorage.getItem('l-token'))._id;
        })


        const theads = ['Title', 'Unit', 'Code','Department','Faculty','Actions']
        const indexes = ['title','unit','code','department','faculty']
        ReactDOM.render(<TableDataForLecturerManageCourse indexes = {indexes} theads = {theads} handleLecture = {this.handleLecture} data = {response} />, document.getElementById('lecturer'))
...
manageNotification = async () => {
    window.document.querySelector("#loader").style.display = "block";
    let token = this.getData('l-token');
    ...
    try {
        ...
        response = res.data.filter((data) => {
            return data.senderID === identity || data.receiverID === identity || data.receiverID == "ALL_LECTURERS";
        });
    ...
    setTimeout(() => {
        ...
        ReactDOM.render(<TableData indexes = {indexes} theads = {theads} data = {response} />, document.getElementById('lecturer'));
    ...
    return (
     ...
     <div className="d-flex" id="wrapper">
        ...
        <div id="page-content-wrapper">
            ...
            <div id = "lecturer" className="container-fluid">
